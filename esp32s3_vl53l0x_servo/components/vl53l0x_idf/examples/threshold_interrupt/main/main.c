...
#define VL53L0X_GPIO_INT  GPIO_NUM_12
...
#define INTER_MEASUREMENT_MS 250
#define THRESHOLD_LOW_MM     400.0
#define THRESHOLD_HIGH_MM    0.0
...
static void gpio_interrupt_init(void)
{
    ...
    ESP_ERROR_CHECK(gpio_isr_handler_add(VL53L0X_GPIO_INT, vl53l0x_gpio_isr, task));
}
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(vl53l0x_set_inter_measurement(sensor, INTER_MEASUREMENT_MS));
    // Generate GPIO interrupt only when the measured distance crosses the configured threshold window.
    ESP_ERROR_CHECK(vl53l0x_set_interrupt_thresholds(sensor, THRESHOLD_LOW_MM, THRESHOLD_HIGH_MM));
    ...
    while (true) {
        // Threshold interrupt mode: wake only on threshold-crossing events and read the corresponding sample.
